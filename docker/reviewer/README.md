...
`docker run` time (see `packages/orchestrator`).
...
flags — the exact flags `packages/orchestrator/src/reviewer.ts` uses for the
...
- **Model + provider — as trailing container ARGV.** They're non-secret and are
  ...
  so they're passed explicitly on the command line rather than via env:
...
- **`OPENROUTER_API_KEY` — via `-e OPENROUTER_API_KEY=...`.** The one input that
  ...
  directly. This is always a short-lived, budget-capped **gateway virtual
  key** (packages/gateway), never a real OpenRouter key — the orchestrator never
...
1. **`OPENROUTER_API_KEY` must be a magpie gateway virtual key** (`sk-magpie-`
   ...
   OpenRouter key (`sk-or-...`) or anything else. The container *always*
   legitimately holds `OPENROUTER_API_KEY` (it's how Pi's OpenRouter provider
   resolves its credential), so the check isn't whether a key is present — it's
...
2. **Network confinement**: a couple of canaries that must be UNREACHABLE (a
   ...
   which is expected to fail at DNS resolution — the container runs
...
**Manual check** (Design D — `--network none` + the mounted gateway
...
A build should be smoke-tested with the following checks:
...
- A hand-run of the full entrypoint against a throwaway worktree + `/out` dir, invoked
  ...
  valid `/out/findings.json` matching the findings schema
  (`packages/orchestrator/src/findings.ts`).
- A `--read-only --tmpfs /tmp --user <uid>:<gid>` run, proving the image doesn't
  depend on writing anywhere but `/tmp` (including `~/.pi/agent/models.json`)
  and the mounted `/out`.
- `docker history --no-trunc magpie-reviewer:0.1.0` showing no secrets baked into any
  layer.

This has been verified empirically, including the provider-override finding, a full
gateway-routed live review through the real `magpie-reviewer` image, and confirmation
the container holds only the per-job virtual key.
