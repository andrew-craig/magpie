...
hardened crun (the shipped default/floor)** — resolved at
...
**Remaining open work:** the M6 nice-to-haves — `@magpie review` on-demand command (`task_ad15`), per-repo `.magpie.toml` (`task_220f`), multi-provider support (`task_9c9d`). M8's own rootless-micro-VM-sandbox epic (`epic_59b1`) retired the earlier M6-E rootless-docker-path direction as superseded. Run `chalk ready` for the current queue.
