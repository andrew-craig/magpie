#!/usr/bin/env bash
# Diagnostic: same real relay + single Node process, but with a 300ms
# delay between successive connections, to test whether the empty-reply
# anomaly is about connection RATE rather than same-process reuse.
set -euo pipefail
...
install -m 0644 "$M8C2_DIR/m8c2-relay-client-delay.mjs" "$ROOTFS/m8c2-relay-client-delay.mjs"

WORKDIR="$(mktemp -d /tmp/magpie-m8c2-delay.XXXXXX)"
JOB_DIR="$WORKDIR/job-delay"
...
python3 "$M8C2_DIR/gw-stub-listener.py" "$UDS_PATH" delay 8 15 >"$LISTENER_LOG" 2>&1 &
LISTENER_PID=$!
...
node /m8c2-relay-client-delay.mjs 5 delayseq 300
echo "=== relay log ==="
cat /tmp/relay.log
'
echo "$GUEST_SCRIPT" > "$ROOTFS/m8c2-diag-delay.sh"
chmod +x "$ROOTFS/m8c2-diag-delay.sh"

echo "=== booting micro-VM ===" >&2
set +e
sg kvm -c "'$BIN' --rootfs '$ROOTFS' --exec /bin/sh --arg /m8c2-diag-delay.sh \
